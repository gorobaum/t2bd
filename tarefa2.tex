...
\end{itemize}
  ...
  \subsection{Modelagens}
	...
	Os dois stored procedures acima são executados de cinco em cinco minutos. SQL Server não suporta triggers usando tempo diretamente, mas podemos fazer uma construção equivalente usando Jobs. 

	Jobs são conjuntos de operações executadas sequencialmente. Um job pode executar diversas atividades, incluindo a execução de scripts T-SQL ou queries. Jobs podem ser escalonados para executarem em algum horário ou alguma frequência desejada.
