...
\section{Introdução}
  ...
  ativos é possível detectar modificações no banco de dados ou criar regras de monitoramento com base no tempo para controlar as transações que modificam o banco de dados. A grande vantagem de utilizar
  um sistema ativo dentro do banco de dados é o ganho de eficiência, já que antes essas operações teriam que ficar na aplicação. Agora o administrador de banco de dados pode ficar responsável em
  criar e manter essas regras, programando na mesma linguagem usada dentro do SGBD.

  Bancos de dados ativos fornecem uma forma de utilizar o paradigma Evento-Condição-Ação integrado ao sistema de gerenciamento de banco de dados. Regras ECA são Ações que acontecem depois de um Evento
  previamente descrito, dado que uma certa Condição seja alcançada pelo banco de dados no seu estado atual. \\
